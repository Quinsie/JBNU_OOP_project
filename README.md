...
#### 진행 상황
* 각 버튼을 누르면 새로운 창이 뜨거나 텍스트가 바뀌게 하는 기능 추가.
* 시간표 버튼과 각각 기능 버튼을 누르면 새로운 창이 뜨도록 함.
    + 이 창에 구현할 추가 기능만 넣어주면 된다.
* 새로운 창을 만들고 나니, 기능에 따라 창들이 달라질 것으로 보인다.
* 창들마다 클래스를 모두 다르게 선언해야 할 필요성 보임.
* 시간표 분석 버튼을 누르면 오른쪽에 분석 결과가 나오게 할 토대 마련. (텍스트가 바뀌도록 했다.)

#### 노트
* https://the-illusionist.me/42 << 버튼 모양 및 스타일 변경 (Graphics 2D 사용)
...
#### 진행 상황
* 시간표가 있는 버튼을 누르면 시간표 수정이 가능한 창을 띄우려고 하였으나, 계획 변경
    + 편의상 시간표는 그냥 띄우기만 하고, 시간표 수정/초기화 버튼을 따로 만들어 붙였다.
* timetableconfig, numberclass, recommendclass, retryclass 윈도우를 모두 따로 생성한다.
